"""
Download monthly median Sentinel-2 composites from Google Earth Engine for the
Brahmanbaria bounding box, split into tiles to stay under the EE direct download
size limits. Each tile is downloaded as a GeoTIFF to a local folder.

Bands included: B2, B3, B4, B5, B6, B7, B8, B8A (suitable for NDVI, EVI,
CIgreen, CIred-edge, MCARI, MTCI).
"""
...
def brahmanbaria_coords() -> list:
    """Return AOI coordinates in lon/lat order."""
    return [
...
def parse_month(month: str) -> int:
    """Accept numeric or month name (e.g., '10' or 'october')."""
    try:
        m = int(month)
        if 1 <= m <= 12:
            return m
...
def init_ee() -> None:
    """Initialize Earth Engine using default auth or service account env vars."""
...
def split_bbox_by_km(
    ...
    """
    Split bounding box into tiles of given width/height in km with optional overlap.
    Returns ((lon_min, lat_min, lon_max, lat_max), (row, col)).
    """
...
def estimate_size_mb_bbox(
    lon_min: float, lat_min: float, lon_max: float, lat_max: float, scale: float, n_bands: int
) -> float:
    """Estimate uncompressed size (MB) for a bbox at given scale."""
    width_deg = lon_max - lon_min
    height_deg = lat_max - lat_min
    mean_lat = (lat_max + lat_min) / 2
    meters_per_deg_lat, meters_per_deg_lon = meters_per_degree(mean_lat)
    width_m = width_deg * meters_per_deg_lon
    height_m = height_deg * meters_per_deg_lat
    pixels = (width_m / scale) * (height_m / scale)
    dtype_bytes = 2  # int16
    return pixels * n_bands * dtype_bytes / (1024**2)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download tiled monthly median Sentinel-2 composite for Brahmanbaria from Earth Engine."
    )
    parser.add_argument("--month", default="october", help="Month (name or number). Default: october.")
    parser.add_argument("--year", type=int, default=2015, help="Year. Default: 2015.")
    parser.add_argument(
        "--scale",
        type=float,
        default=10,
...
def main() -> None:
    ...
    print(
        f"Downloading {total_tiles} tiles at {args.scale} m for {args.year}-{month_num:02d} "
    ...
    for idx, ((lon_min, lat_min, lon_max, lat_max), (r, c)) in enumerate(tiles, start=1):
        est_mb = estimate_size_mb_bbox(lon_min, lat_min, lon_max, lat_max, args.scale, n_bands)
        if est_mb > 45:
            print(
                f"Tile r{r}c{c} estimated ~{est_mb:.1f} MB; may exceed EE direct download cap. "
                "Consider reducing --tile-width-km/--tile-height-km or using a coarser --scale."
            )
