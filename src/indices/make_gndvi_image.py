"""
Compute GNDVI from separate Sentinel-2 band rasters (B03 green, B08 NIR)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compute GNDVI from separate Sentinel-2 band rasters.")
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output GNDVI GeoTIFF path. Default: map/<year>/gndvi_<yyyymmdd>.tif based on matched date.",
    )
    parser.add_argument(
        "--scale",
        type=float,
...
def compute_gndvi(green: np.ndarray, nir: np.ndarray, nodata_mask: np.ndarray | None = None, scale: float = 1.0) -> np.ndarray:
    ...
    gndvi = np.empty_like(green, dtype="float32")
    valid = ~mask
    gndvi[valid] = (nir[valid] - green[valid]) / denom[valid]
    gndvi[mask] = np.nan
    # clip implausible
    implausible = (gndvi < -2) | (gndvi > 2)
    gndvi[implausible] = np.nan
    return gndvi
...
def main() -> None:
    ...
    if args.output:
        ...
        out_path = Path("map") / str(args.year) / f"gndvi_{date_str}.tif"
    ...
    with rasterio.open(green_path) as green_src, rasterio.open(nir_path) as nir_src:
        ...
        with rasterio.open(out_path, "w", **profile) as dst:
            for _, window in green_src.block_windows(1):
                ...
                gndvi_block = compute_gndvi(green, nir, nodata_mask, scale=args.scale)
                dst.write(gndvi_block.astype("float32"), 1, window=window)

    print(f"Saved GNDVI to {out_path} (year {args.year}, date {date_str}, scale {args.scale})")
