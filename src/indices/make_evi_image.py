"""
Compute EVI from separate Sentinel-2 band rasters (B02 blue, B03 green, B04 red, B08 NIR)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compute EVI from separate Sentinel-2 band rasters.")
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output EVI GeoTIFF path. Default: map/<year>/evi_<yyyymmdd>.tif based on matched date.",
    )
    parser.add_argument(
        "--scale",
        type=float,
...
def find_band_paths(year: int, date_filter: Optional[str]) -> Dict[str, Path]:
...
def compute_evi(blue: np.ndarray, red: np.ndarray, nir: np.ndarray, nodata_mask: np.ndarray | None = None, scale: float = 1.0) -> np.ndarray:
    blue = blue.astype("float32") * scale
    red = red.astype("float32") * scale
    nir = nir.astype("float32") * scale
    # Mask invalid reflectance ranges
    invalid_ref = (blue < 0) | (red < 0) | (nir < 0) | (blue > 1.5) | (red > 1.5) | (nir > 1.5)
...
def main() -> None:
    ...
    blue_path = band_paths["B02"]
    red_path = band_paths["B04"]
    ...
    if args.output:
        ...
        out_path = Path("map") / str(args.year) / f"evi_{date_str}.tif"

    with rasterio.open(red_path) as red_src, rasterio.open(blue_path) as blue_src, rasterio.open(
        nir_path
    ) as nir_src:
        # ensure alignment
        if not (red_src.crs == blue_src.crs == nir_src.crs and red_src.transform == blue_src.transform == nir_src.transform):
        ...
        with rasterio.open(out_path, "w", **profile) as dst:
            for _, window in red_src.block_windows(1):
                ...
                masks = []
                for band, nod in zip((red, blue, nir), (red_src.nodata, blue_src.nodata, nir_src.nodata)):
                    if nod is not None:
                        masks.append(band == nod)
                    # Mask obvious invalid DN
    ...
    print(f"Saved EVI to {out_path} (year {args.year}, date {date_str}, scale {args.scale})")
