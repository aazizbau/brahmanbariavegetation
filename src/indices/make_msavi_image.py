"""
Compute MSAVI from separate Sentinel-2 band rasters (B04 red, B08 NIR)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compute MSAVI from separate Sentinel-2 band rasters.")
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output MSAVI GeoTIFF path. Default: map/<year>/msavi_<yyyymmdd>.tif based on matched date.",
    )
    parser.add_argument(
        "--scale",
        type=float,
...
def compute_msavi(red: np.ndarray, nir: np.ndarray, nodata_mask: np.ndarray | None = None, scale: float = 1.0) -> np.ndarray:
    red = red.astype("float32") * scale
    nir = nir.astype("float32") * scale
    invalid_ref = (red < 0) | (nir < 0) | (red > 1.5) | (nir > 1.5)
    term = (2 * nir + 1) ** 2 - 8 * (nir - red)
    term = np.where(term < 0, 0, term)  # avoid negative inside sqrt
    sqrt_term = np.sqrt(term)
    msavi = (2 * nir + 1 - sqrt_term) / 2
    mask = invalid_ref
    if nodata_mask is not None:
        mask = mask | nodata_mask
    msavi = np.where(mask, np.nan, msavi.astype("float32"))
    # clip implausible
    implausible = (msavi < -1) | (msavi > 1)
    msavi[implausible] = np.nan
    return msavi
...
def main() -> None:
    ...
    if args.output:
        ...
        out_path = Path("map") / str(args.year) / f"msavi_{date_str}.tif"

    with rasterio.open(red_path) as red_src, rasterio.open(nir_path) as nir_src:
        if not (red_src.crs == nir_src.crs and red_src.transform == nir_src.transform):
        ...
        with rasterio.open(out_path, "w", **profile) as dst:
            for _, window in red_src.block_windows(1):
                ...
                masks = []
                for band, nod in zip((red, nir), (red_src.nodata, nir_src.nodata)):
                    if nod is not None:
                        masks.append(band == nod)
                    masks.append((band <= 0) | (band > 12000))
                nodata_mask = np.logical_or.reduce(masks) if masks else None
                msavi_block = compute_msavi(red, nir, nodata_mask, scale=args.scale)
                dst.write(msavi_block.astype("float32"), 1, window=window)

    print(f"Saved MSAVI to {out_path} (year {args.year}, date {date_str}, scale {args.scale})")
